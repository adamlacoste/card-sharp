 ...
 *
 *  @module card-sharp/lib/deck
 *
 *  @requires assert
 *  @requires card-sharp/lib/card
 *  @requires knuth-shuffle
 */
...
class Deck {
    ...
    remove(cards = []) {
        ...
        for (let card of cards) {
            let card_index    = this.cards.indexOf(card),
                discard_index = this.discards.indexOf(card);
            ...
            if (discard_index >= 0) {
                this.discards.splice(discard_index, 1);
            } // end if
        } // end for-of
     ...
     */
    shuffle(include_discards = false) {
